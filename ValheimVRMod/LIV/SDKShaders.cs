        ...
        public class ShaderCache {
            public ShaderCache(AssetBundle bundle) {
                try {
                    ...
                    if (!state)
                        Debug.Log($"Failed to retreive at least one of the LIV shaders from {bundle.name}");
                } catch (Exception e) {
                    Debug.Log($"Failed to initialize the ShaderCache from asset bundle {bundle.name} : {e}");
                    state = false;
                }
            }
